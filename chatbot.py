...
class chatbot:
    ...
    def createdata(self):
        ...
        self.words = [stemmer.stem(w.lower())
                      for w in self.words if w not in '?']
    ...
    def create_model(self):
        ...
        self.model.add(Dense(128, input_shape=(
            len(self.train_x[0]),), activation='relu'))
    ...
    def train_model(self):
        self.model.fit(np.array(self.train_x), np.array(
            self.train_y), epochs=500, batch_size=5, verbose=1)
    ...
    def classify_local(self, sentence):
        ...
        input_data = pd.DataFrame(
            [self.bow(sentence, self.words)], dtype=float, index=['input'])
